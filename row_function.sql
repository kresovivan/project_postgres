...
/*Очистка данных
  проверим не дублируются ли дни в таблице weather */
...
/*Чтобы дубли не портили отчеты придется их почистить
  пронумеруем дни с помощью row_number
*/
...
WITH src AS (SELECT wdate,
                    ...
                    precip,
                    pressure,
                    ROW_NUMBER() OVER (PARTITION BY wdate) AS rn
             FROM weather)

---получаем записи без дублей
SELECT wdate,
       wtemp,
       precip,
       pressure,
