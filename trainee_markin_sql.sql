...
в столбец первичного ключа. Для предотвращения такой ситуации при
получении очередного уникального значения первичного ключа следует
использовать последовательность (см. лекц. 6.3).
...
следующем примере. Требуется обновить таблицу Services из таблицы
Services_New (рис. 7.12), т.е. те услуги, которые уже присутствуют
...
должны быть добавлены. Запрос слияния:

MERGE INTO Services S
USING Services_New S_New
ON S.Serviceid = S_New.Serviceid
WHEN NOT MATCHED BY SOURCE THEN
    -- удаление строк, отсутствующих в Services_New
    DELETE
WHEN NOT MATCHED BY TARGET THEN
    -- добавление новых строк
    INSERT VALUES (Serviceid, Servicenm)
