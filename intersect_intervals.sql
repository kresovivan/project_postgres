SELECT 
    tsrange('2024-01-01 00:00:00', '2024-01-10 23:59:59', '[]') as range1,
    tsrange('2024-01-05 00:00:00', '2024-01-15 23:59:59', '[]') as range2,
    LOWER(tsrange('2024-01-01 00:00:00', '2024-01-10 23:59:59', '[]') * 
          tsrange('2024-01-05 00:00:00', '2024-01-15 23:59:59', '[]')) as overlap_start,
    UPPER(tsrange('2024-01-01 00:00:00', '2024-01-10 23:59:59', '[]') * 
          tsrange('2024-01-05 00:00:00', '2024-01-15 23:59:59', '[]')) as overlap_end;

/*
Что происходит на самом деле:
Ввод: daterange('2024-01-01', '2024-01-10', '[]')
Хранится: [2024-01-01, 2024-01-11) - верхняя граница смещена на +1 день
Отображается: [2024-01-01, 2024-01-10] - но при выводе показывается как включительная
*/

WITH ranges AS (
    SELECT 
        daterange('2024-01-01', '2024-01-10', '[]') as range1,
        daterange('2024-01-05', '2024-01-15', '[]') as range2
)
SELECT 
    range1,
    range2,
    LOWER(range1 * range2) as overlap_start,
    (UPPER(range1 * range2) - INTERVAL '1 day')::date as overlap_end
FROM ranges;

WITH intervals AS (
    SELECT 
        daterange('2024-01-01', '2024-02-01', '[]') as range1,
        daterange('2024-01-15', '2024-02-15', '[]') as range2,
        daterange('2024-01-20', '2024-03-01', '[]') as range3
)
SELECT 
    LOWER(range1 * range2 * range3) as triple_overlap_start,
    UPPER(range1 * range2 * range3) as triple_overlap_end
FROM intervals;
...
-- Таблица счетов ACC_ACC
CREATE TABLE ACC_ACC (
    acc_id SERIAL PRIMARY KEY,
    acc VARCHAR(50) NOT NULL,              -- Номер счета
    client_id INTEGER NOT NULL,            -- ID клиента
    date_beg DATE NOT NULL,                -- Дата открытия счета
    date_end DATE NOT NULL,                -- Дата закрытия счета (9999-12-31 если открыт)
    type_acc VARCHAR(30) NOT NULL,         -- Тип счета
    currency VARCHAR(3) DEFAULT 'RUB',     -- Валюта счета
    branch VARCHAR(20),                    -- Отделение банка
    CONSTRAINT chk_date_range CHECK (date_end >= date_beg),
    CONSTRAINT chk_open_account CHECK (date_end = '9999-12-31' OR date_end < '9999-12-31')
);

-- Таблица проводок TRN
CREATE TABLE TRN (
    trn_id SERIAL PRIMARY KEY,
    acc VARCHAR(50) NOT NULL,              -- Номер счета
    date_trn DATE NOT NULL,                -- Дата проводки
    summa_trn DECIMAL(15,2) NOT NULL,      -- Сумма проводки
    operation_type VARCHAR(50),            -- Тип операции
    counterparty VARCHAR(100),             -- Контрагент
    description TEXT,                      -- Описание операции
    trn_status VARCHAR(20) DEFAULT 'POSTED' -- Статус проводки
);

-- Индексы для оптимизации
CREATE INDEX idx_acc_acc_dates ON ACC_ACC (acc, date_beg, date_end);
CREATE INDEX idx_acc_acc_client ON ACC_ACC (client_id, date_beg);
CREATE INDEX idx_trn_acc_date ON TRN (acc, date_trn);
CREATE INDEX idx_trn_date ON TRN (date_trn);

-- Вставка данных в таблицу счетов ACC_ACC
INSERT INTO ACC_ACC (acc, client_id, date_beg, date_end, type_acc, currency, branch) VALUES
-- Клиент 1: Два текущих счета (один закрыт, один открыт)
('40702810100010000001', 1, '2023-01-15', '2023-12-31', 'CURRENT', 'RUB', 'MOSCOW'),
('40702810100010000002', 1, '2024-01-01', '9999-12-31', 'CURRENT', 'RUB', 'MOSCOW'),

-- Клиент 2: Текущий счет + депозит
('40702810200020000001', 2, '2023-03-10', '9999-12-31', 'CURRENT', 'RUB', 'SPB'),
('42301810200020000001', 2, '2024-02-01', '2024-08-01', 'DEPOSIT', 'RUB', 'SPB'),

-- Клиент 3: Закрытый текущий счет
('40702810300030000001', 3, '2023-06-01', '2024-01-31', 'CURRENT', 'RUB', 'EKATERINBURG'),

-- Клиент 4: Два открытых счета (текущий + карточный)
('40817810400040000001', 4, '2023-11-15', '9999-12-31', 'CURRENT', 'RUB', 'NOVOSIBIRSK'),
('40817810400040000002', 4, '2024-03-01', '9999-12-31', 'CREDIT_CARD', 'RUB', 'NOVOSIBIRSK'),

-- Клиент 5: Счет с коротким периодом
('40702810500050000001', 5, '2024-01-15', '2024-04-15', 'CURRENT', 'USD', 'MOSCOW'),

-- Клиент 6: Долгосрочный открытый счет
('40702810600060000001', 6, '2022-05-20', '9999-12-31', 'CURRENT', 'EUR', 'SPB');

-- Вставка данных в таблицу проводок TRN
INSERT INTO TRN (acc, date_trn, summa_trn, operation_type, counterparty, description) VALUES
-- Проводки для счета 40702810100010000001 (клиент 1, закрытый счет)
('40702810100010000001', '2023-02-10', 50000.00, 'INCOME', 'ООО "Компания А"', 'Зарплата'),
('40702810100010000001', '2023-02-15', -15000.00, 'PAYMENT', 'ПАО "Газпром"', 'Оплата коммунальных услуг'),
('40702810100010000001', '2023-03-01', 75000.00, 'INCOME', 'ИП Сидоров', 'Оплата по договору'),
('40702810100010000001', '2023-11-20', -25000.00, 'TRANSFER', '40702810200020000001', 'Перевод между счетами'),

-- Проводки для счета 40702810100010000002 (клиент 1, открытый счет)
('40702810100010000002', '2024-01-05', 100000.00, 'INCOME', 'ООО "Компания А"', 'Зарплата'),
('40702810100010000002', '2024-01-10', -30000.00, 'PAYMENT', 'Яндекс.Такси', 'Оплата такси'),
('40702810100010000002', '2024-02-15', 80000.00, 'INCOME', 'Фриланс', 'Оплата услуг'),
('40702810100010000002', '2024-03-20', -45000.00, 'PAYMENT', 'Авито', 'Покупка товаров'),

-- Проводки для счета 40702810200020000001 (клиент 2, текущий счет)
('40702810200020000001', '2024-01-20', 150000.00, 'INCOME', 'ООО "Рога и копыта"', 'Аванс'),
('40702810200020000001', '2024-02-05', -50000.00, 'TRANSFER', '42301810200020000001', 'Пополнение депозита'),
('40702810200020000001', '2024-03-10', 120000.00, 'INCOME', 'ООО "Рога и копыта"', 'Зарплата'),

-- Проводки для депозита клиента 2
('42301810200020000001', '2024-02-05', 50000.00, 'DEPOSIT', '40702810200020000001', 'Открытие депозита'),

-- Проводки для закрытого счета клиента 3
('40702810300030000001', '2023-07-15', 200000.00, 'INCOME', 'ПАО "Сбербанк"', 'Кредит'),
('40702810300030000001', '2023-08-20', -75000.00, 'PAYMENT', 'ООО "Стройматериалы"', 'Покупка материалов'),

-- Проводки для клиента 4 (текущий счет)
('40817810400040000001', '2024-01-25', 80000.00, 'INCOME', 'Иванов И.И.', 'Возврат долга'),
('40817810400040000001', '2024-02-14', -20000.00, 'PAYMENT', 'ООО "МТС"', 'Оплата связи'),

-- Проводки для клиента 4 (карточный счет)
('40817810400040000002', '2024-03-05', -15000.00, 'PAYMENT', 'Магнит', 'Покупка продуктов'),
('40817810400040000002', '2024-03-15', -8000.00, 'PAYMENT', 'АЗС Лукойл', 'Заправка'),

-- Проводки для клиента 5 (короткий период)
('40702810500050000001', '2024-02-01', 5000.00, 'INCOME', 'Обмен валют', 'Конвертация'),
('40702810500050000001', '2024-03-10', -2000.00, 'PAYMENT', 'Amazon', 'Онлайн покупка'),

-- Проводки для клиента 6 (долгосрочный счет)
('40702810600060000001', '2023-06-15', 10000.00, 'INCOME', 'TransferWise', 'Международный перевод'),
('40702810600060000001', '2023-12-20', -5000.00, 'PAYMENT', 'Booking.com', 'Отель'),
('40702810600060000001', '2024-02-28', 15000.00, 'INCOME', 'Freelance', 'Удаленная работа');

-- Проводки вне периодов действия счетов (для тестирования)
INSERT INTO TRN (acc, date_trn, summa_trn, operation_type, counterparty, description) VALUES
('40702810100010000001', '2024-01-15', 10000.00, 'TEST', 'Тест', 'Проводка после закрытия счета'),
('40702810300030000001', '2024-03-01', 5000.00, 'TEST', 'Тест', 'Проводка после закрытия счета');



-- Несогласованный подход:
WHERE (trn.date_trn >= acc.date_beg AND trn.date_trn <= acc.date_end)  -- для закрытых
   OR (trn.date_trn >= acc.date_beg AND acc.date_end = '9999-12-31')   -- для открытых

-- Единый подход с диапазонами:
WHERE trn.date_trn BETWEEN 
    LOWER(daterange(acc.date_beg, acc.date_end, '[]')) 
AND UPPER(daterange(acc.date_beg, acc.date_end, '[]'))

-- Вариант 1: '[]' - включительно-включительно
daterange('2024-01-01', '2024-12-31', '[]')
-- LOWER: '2024-01-01' ✅ (1 января ВКЛЮЧЕНО)
-- UPPER: '2024-12-31' ✅ (31 декабря ВКЛЮЧЕНО)
    
-- Создание представления для упрощения доступа к данным пересечения
CREATE OR REPLACE VIEW VW_ACC_TRN_INTERSECTION AS
SELECT 
    ap.acc_id,
    ap.acc,
    ap.client_id,
    ...
    t.description,
    t.trn_status,
    ap.date_beg as account_open_date,
    ap.date_end as account_close_date,
    -- Ключевые метрики для аналитики
    CASE 
        WHEN t.summa_trn > 0 THEN 'INCOME'
        WHEN t.summa_trn < 0 THEN 'EXPENSE'
        ELSE 'ZERO'
    END as cash_flow_direction,
    EXTRACT(MONTH FROM t.date_trn) as transaction_month,
    EXTRACT(YEAR FROM t.date_trn) as transaction_year
FROM (
    SELECT 
        acc_id,
        acc,
        ...
        currency,
        branch,
        daterange(date_beg, date_end, '[]') as acc_date_range,
        LOWER(daterange(date_beg, date_end, '[]')) as range_start,
        UPPER(daterange(date_beg, date_end, '[]')) as range_end,
        CASE WHEN date_end = '9999-12-31' THEN 'OPEN' ELSE 'CLOSED' END as account_status
    FROM ACC_ACC
) ap
JOIN TRN t ON ap.acc = t.acc
WHERE t.date_trn BETWEEN ap.range_start AND ap.range_end;

SELECT * FROM VW_ACC_TRN_INTERSECTION 
WHERE client_id = 1
ORDER BY date_trn;


-- Пример использования представления
SELECT acc_id, acc, trn_id, date_trn, account_open_date, account_close_date 
FROM VW_ACC_TRN_INTERSECTION 
WHERE client_id in (1,2)
ORDER BY date_trn, trn_id;
...
----Этот запрос решает бизнес-задачу: "
----Найти балансы счетов клиентов в те периоды, когда счет был открыт 
----и одновременно действовал кредитный продукт".
-- Если бизнес-логика позволяет, проще использовать исключительные границы
SELECT 
    a.client_id,
    a.account_number,
    a.account_type,
    ...
    c.start_date, 
    c.end_date,
    daterange(a.open_date, a.close_date, '[)') as account_range,  -- Исключительная верхняя граница
    daterange(c.start_date, c.end_date, '[)') as credit_range,
    (daterange(a.open_date, a.close_date, '[)') * daterange(c.start_date, c.end_date, '[)')) as overlap_range
FROM acc a 
JOIN cdh c ON a.client_id = c.client_id AND a.account_number = c.account_number 
JOIN saldo s ON a.account_number = s.account_number
...
AND a.account_number = '45201810100010000002'
ORDER BY overlap_range;
/*Комбинирование grouping sets, cube, rollup*/

SELECT
  a.account_id,
  l.loan_id,
  lower(overlap_period) AS overlap_start,
  upper(overlap_period) AS overlap_end
FROM (
  SELECT
    a.account_id,
    l.loan_id,
    daterange(a.valid_from, a.valid_to, '[]')
      * daterange(l.loan_from, l.loan_to, '[]') AS overlap_period
  FROM account a
  JOIN loan_agreement l
    ON a.account_id = l.account_id
  WHERE
    daterange(a.valid_from, a.valid_to, '[]')
      &&
    daterange(l.loan_from, l.loan_to, '[]')
) sub
WHERE overlap_period IS NOT NULL;
